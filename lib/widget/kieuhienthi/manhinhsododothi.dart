...
class _ListviewSodoDothiState extends State<ListviewSodoDothi> {
  ...
  void initState() {
    ...
    _subscriptionCambien =
      ...
      // });
          setState(() {
            // Nếu color == null thì là tín hiệu xóa
            if (cambien.color == null) {
              listCambiendothi.removeWhere((c) =>
              c.id == cambien.id && c.name == cambien.name);
            } else {
              listCambiendothi.removeWhere((c) =>
  ...
  Widget getRow(int index) {
    final cambien = listCambiendothi[index];
    final parts = cambien.id.split(',');
    final maCambien = parts.first;
    final mamau = cambien.color ?? Colors.grey;

    return DongSodoDothi(
      tenCambien: '${cambien.name} (${cambien.donvi})',
      maCambien: maCambien,
      mamau: mamau,
    );
  }
